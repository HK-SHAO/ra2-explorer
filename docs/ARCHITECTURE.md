...
单位 grid 只为可见区域及邻近滚动范围请求缩略图，并按渲染成本限制并发；VXL 卡片以较高权重占用队列，详情模型加载期间暂停新的卡片任务，避免大量 Python 体素投影与模型响应争用。步兵缩略图保留额外透明边距，不再把单个 SHP 人物放大到接近整张卡片。三维场景直接写入实例矩阵与颜色缓冲，复用服务端按实际体素计算的包围盒；VPL 已提供最终颜色时使用无冗余法线和 UV 的紧凑立方体几何，并确保首次取景只触发一次重绘。
