...
Web UI 使用统一树复用来源、搜索和详情交互，不再保留顶部工具栏。左栏顶部只显示 RA2 Explorer，底部固定显示设置；载具、航空器、步兵和建筑是“单位”的子节点，动画继续按 SHP/HVA/视频分层，语音与音效继续按游戏事件语义分层；设置中启用的地图、图像、地形和规则文本是同级节点，不存在平行的“资源 / 单位”模式。集合区依据节点显示有效排序；list/grid 共享查询、筛选、选中和分页状态，单位类型由树节点决定，集合内只保留阵营筛选。默认工作台是上方集合、下方宽屏详情；设置可切为集合与窄屏详情左右排列，弹出的独立详情始终使用宽屏排版和自身滚动容器。不同格式分别进入三维、动画顺序/全帧网格、图像、单击播放音频、视频、地图或文本视图。单位详情把声音和动画分开，声音按选中、移动、攻击等事件分组并显示原文/中文，关联动画在主预览中播放而不充当下载入口。VXL 单体和组合单位使用 WebGL 真三维视图，光标可以旋转、缩放和平移。来源的 `scanned_at` 变化会同时刷新索引视图并切换派生缓存版本。
