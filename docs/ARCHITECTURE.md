...
| VXL | 体素列解码、等距 PNG、交互式 WebGL 三维模型 | 只发送表面体素并保留调色板索引、法线索引及法线模式；Three.js 使用游戏 PAL、玩家换色、正交相机和受光材质，支持鼠标旋转、滚轮缩放与平移 |
...
配色表（PAL）是 RA2 索引色资源的 256 色查找表：SHP/VXL 像素或体素保存的是颜色索引，而不是现代模型的位图贴图。TMP 使用 `iso*.pal`，SHP/VXL 使用 `unit*.pal`；具体 TEM/SNO/URB/UBN/LUN/DES 变体优先从资产所在剧场归档判断。建筑 SHP 即使来自 `SNOW.MIX` 也使用 `unitsno.pal`，不能因其等距布局误用 `isosno.pal`。VXL 场景契约 v3 额外保留每个体素的法线索引和法线模式；当前 WebGL 视图用游戏调色板、玩家换色、正交等距相机、环境光和方向光恢复材质层次。查看器从实际变换后的体素边界重新计算三维包围盒，把八个角投影到当前相机平面后同时适配横向与纵向视野；容器尺寸变化和重置视角都会重新适配完整模型。尚未逐项复刻 Westwood VPL 的法线亮度查找表。
...
Web UI 使用统一树复用来源、搜索和详情交互，不再保留顶部工具栏。左栏顶部只显示 RA2 Explorer，底部固定显示设置，并可收起为保留可访问标签的图标栏；载具、航空器、步兵和建筑是“单位”的子节点，动画继续按 SHP/HVA/视频分层，语音与音效继续按游戏事件语义分层；设置中启用的地图、图像、地形和规则文本是同级节点，不存在平行的“资源 / 单位”模式。集合区依据节点显示有效排序；list 采用双列，grid 采用内容卡片，两者共享查询、筛选、选中和分页状态，单位类型由树节点决定，集合内只保留阵营筛选。默认工作台是上方集合、下方宽屏详情；设置可切为集合与窄屏详情左右排列，两种方向的分隔条都支持指针拖动和键盘调节，弹出的独立详情始终使用宽屏排版和自身滚动容器。浏览上下文、选择、布局尺寸与滚动位置保存在浏览器本地并在刷新后恢复。单位详情标题位于信息列上方，规则属性依据内容宽度在一至三列间跨列。不同格式分别进入三维、动画顺序/全帧网格、图像、单击播放音频、视频、地图或文本视图。单位详情把声音和动画分开；没有原文或中文时不生成空占位行。来源的 `scanned_at` 变化会同时刷新索引视图并切换派生缓存版本。
