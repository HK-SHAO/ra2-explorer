...
- 同一官方安装可建立 3,322 条去重语义音频记录：2,533 条语音、772 条音效和 17 条尚无足够配置证据的未分类项。修正 `Russian` 字段后可确认 935 个 EVA 资产，补齐单位事件名称后其中 808 个已有文本；剩余 127 个主要是官方配置不带字幕的尤里复仇任务/合作任务对白和 Dummy。1,309 个单位语音中已有 1,209 个带文本，剩余项主要是动物、无线电、Dummy 或不含人声的效果。分类使用单位 Voice 字段、EVA、SOUND、CSF 与转录引用的联合证据，不以 BAG/WAV/AUD 文件载体推断用途。
