...
- MIX 松散条目通过 seek/read 读取，嵌套归档通过 mmap 视图遍历；BAG 虚拟音频只读取指定偏移片段。模型场景首次生成后保存为紧凑 JSON，命中时直接返回原始缓存字节并允许本机浏览器短期复用，不再重复解析与序列化；模型实现或主体解析规则改变时必须提升派生缓存及前端 URL 版本。过长的派生缓存身份会压缩为带稳定摘要的短文件名，给原子写入临时后缀保留 Windows 路径空间。格式解码仍有单资产防御性上限，尚无持久任务队列。
