...
- 同一官方安装可建立 3,322 条去重语义音频记录：2,532 条语音、773 条音效和 17 条尚无足够配置证据的未分类项。修正 `Russian` 字段及 EVAMD 事件语义后可确认 616 个真正的 EVA 播报，616 个均有可验证文本；334 个任务对白中 209 个已有文本，剩余 125 个是官方配置不带字幕、且尚未完成校对转录的尤里复仇任务/合作任务对白。`SPSYREAD.WAV` 是无台词的心灵揭示就绪提示音，`DUMMY.WAV` 只有约 0.06 秒且不进入播报分类。1,309 个单位语音中已有 1,209 个带文本，剩余项主要是动物、无线电、Dummy 或不含人声的效果。分类使用单位 Voice 字段、EVA、SOUND、CSF 与转录引用的联合证据，不以 BAG/WAV/AUD 文件载体推断用途。
