...
`TechLevel=-1` 但具有 `ConstructionYard` 或 `UndeploysInto` 的建造场按玩家部署建筑处理，不因 `Capturable` 被误分为中立科技。集合中的单位 grid 卡片只保留缩略图和名称；声音分组 header 将主标题与 ID/附加信息放在同一行并独立省略。宽屏详情使用确定的 `minmax(0, 1fr)` Grid 行限制预览列；右侧长内容只增加详情滚动范围，不再把 WebGL mount 和 canvas 撑高或形成 ResizeObserver 尺寸反馈。
