...
| VXL | 体素列解码、等距 PNG、交互式 WebGL 三维模型 | 只发送表面体素并保留调色板索引、法线索引及法线模式；Three.js 使用游戏 PAL、玩家换色和受光材质，支持鼠标旋转、滚轮缩放与平移 |
...
声音关系从 RULES/ART 的语音、移动、部署、武器报告和动画字段继续追踪到 SOUNDMD/EVA、AUDIO.IDX/BAG、WAV/AUD 与 CSF。语音和音效以这些引用证据分类，容器格式不参与语义判断，因此 BAG 中的战斗音效和松散 WAV 中的台词不会再被颠倒。带 `VOX:` 本地化项的语音会同时返回台词文本；没有 CSF 台词的战役片段会从稳定文件名生成任务/段落说明。独立媒体目录按单位语音、EVA 播报、战役语音、战斗音效、单位音效和环境音效分组，并保留事件、单位、阵营和国家反向关系。关系是从官方配置计算出的可重建视图，不写回源文件。

配色表（PAL）是 RA2 索引色资源的 256 色查找表：SHP/VXL 像素或体素保存的是颜色索引，而不是现代模型的位图贴图。TMP 使用 `iso*.pal`，SHP/VXL 使用 `unit*.pal`；具体 TEM/SNO/URB/UBN/LUN/DES 变体优先从资产所在剧场归档判断。建筑 SHP 即使来自 `SNOW.MIX` 也使用 `unitsno.pal`，不能因其等距布局误用 `isosno.pal`。VXL 场景契约 v3 额外保留每个体素的法线索引和法线模式；当前 WebGL 视图用游戏调色板、玩家换色、半球光和方向光恢复材质层次，尚未逐项复刻 Westwood VPL 的法线亮度查找表。
...
Web UI 在三栏工作台中复用来源、搜索和详情交互。左栏是统一树：载具、航空器、步兵和建筑是“单位”的子节点；动画继续按 SHP/HVA/视频分层，语音与音效继续按游戏事件语义分层；设置中启用的地图、图像、地形和规则文本是同级节点，不存在平行的“资源 / 单位”模式。中栏依据节点显示格式或语义标签，并提供服务端稳定排序；list/grid 共享查询、筛选、排序、选中和分页状态。单位可分别按类型、阵营与国家组合筛选。不同格式分别进入三维、动画顺序/全帧网格、图像、单击播放音频、视频、地图或文本视图；右侧详情可以弹出到独立窗口。VXL 单体和组合单位使用 WebGL 真三维视图，光标可以旋转、缩放和平移。来源的 `scanned_at` 变化会同时刷新索引视图并切换派生缓存版本。
...
- `GET /api/assets/{id}/model.json`：读取 VXL 或同名 VXL/HVA 的 v3 浏览器三维场景，可选择 HVA 帧并保留调色板与法线语义；
- `GET /api/entities`：按来源、名称、类型和可预览状态检索规则实体，并返回阵营与国家 facets；
- `GET /api/media`：按语音/音效、游戏事件分组、说明或文件名分页检索语义媒体；
...
- 同一官方安装可建立 3,322 条去重语义音频记录：2,317 条语音、958 条音效和 47 条尚无足够配置证据的未分类项。分类使用单位 Voice 字段、EVA、SOUND 和 CSF 引用的联合证据，不以 BAG/WAV/AUD 文件载体推断用途。
