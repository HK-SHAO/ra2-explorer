...
Web UI 在现有三栏工作台中复用来源、搜索和详情交互：“游戏单位”默认显示载具，可切换步兵、航空器、建筑或全部实体，并按可预览状态筛选。语义覆盖卡显示组件、本地化和依赖状态；详情提供帧播放、VXL 朝向、阵营色、调色板、武器链及组件原始导出。资源与单位搜索状态彼此独立，详情请求切换时先清空旧实体；来源的 `scanned_at` 变化会使统计、列表、诊断和详情一并刷新，避免重扫后继续展示旧索引。
