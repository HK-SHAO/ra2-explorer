...
站点不会启动时下载整套 139.2 MiB 数据，也不会把发布 ZIP 或 npm tarball 发送给访客。单位页启动只读取 482,710 字节的当前语言单位目录和当前分类的一张图集；本次车辆图集为 193,492 字节，把原先冷启动的 45 条缩略图请求合并为 1 条，并由 HTML 在应用启动前预取。几 KB 的快照清单直接携带声音类型、用途和事件计数，因此侧栏无需预读约 2.4 MiB 的当前语言声音目录。交互模型、详情和声音仍按操作加载；搜索时单位结果先到先显示，不等待较大的声音索引。
