...
| 声音 | 3,322 | — |
| 发布文件 | 32,801 | — |
| 解包后的 Pages 数据 | 199,062,644 字节 | 约 189.8 MiB |
| 固定数据 ZIP | 131,453,873 字节 | 约 125.4 MiB |
| 四个 npm/jsDelivr 按需分包 | 132,541,555 字节 | 约 126.4 MiB |
...
- 启动目录、单位详情、动画 WebP、卡片图集与搜索图集优先走固定版本 jsDelivr，Pages 保留同源副本并仅在失败时回退，不用竞速请求制造双倍流量；动画首帧与有限前瞻帧按播放优先级解码，长动画不会一次性争抢全部连接；
