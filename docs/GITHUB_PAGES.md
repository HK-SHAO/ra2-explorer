...
| 固定数据 ZIP | 80,410,025 字节 | 76.7 MiB |
| ZIP SHA-256 | `ba1f0103c6e7a857230806cef0841ad9e40b47633562682a3711ac93a0a989cd` | — |
...
站点不会启动时下载整套 139.2 MiB 数据，也不会把发布 ZIP 发送给访客。单位页启动只读取约 471 KiB 的当前语言单位目录和当前分类的一张图集，不再为了侧栏计数提前读取约 2.4 MiB 的声音目录；本次车辆图集约 189 KiB，把原先冷启动的 45 条缩略图请求合并为 1 条。交互模型、详情和声音仍按操作加载；搜索时单位结果先到先显示，不等待较大的声音索引。不同分类、停留时间、CDN 压缩和浏览器缓存都会改变会话流量，因此不把某一次本机自动化数字当成公开带宽承诺；完整遍历所有资源的理论上界约为 139.2 MiB。
