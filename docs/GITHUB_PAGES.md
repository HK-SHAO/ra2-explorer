...
| 声音 | 3,322 | — |
| 发布文件 | 28,977 | — |
| 解包后的 Pages 数据 | 145,948,266 字节 | 139.2 MiB |
| 固定数据 ZIP | 80,410,647 字节 | 76.7 MiB |
| ZIP SHA-256 | `878a6703df91b37e5794789a690e592a73968ae96ecf6ac1782a23632be878ff` | — |
...
| `catalog` | 5,978,070 | 简体/繁体单位与声音目录、图集位置及搜索别名 |

站点不会启动时下载整套 139.2 MiB 数据，也不会把发布 ZIP 发送给访客。单位页启动只读取约 471 KiB 的当前语言单位目录和当前分类的一张图集，不再为了侧栏计数提前读取约 2.4 MiB 的声音目录；本次车辆图集约 189 KiB，把原先冷启动的 45 条缩略图请求合并为 1 条，并由 HTML 在应用启动前预取。几 KB 的快照清单直接携带声音类型、用途和事件计数，因此分类面板无需等待声音目录。交互模型、详情和声音仍按操作加载；搜索时单位结果先到先显示，不等待较大的声音索引。不同分类、停留时间、CDN 压缩和浏览器缓存都会改变会话流量，因此不把某一次本机自动化数字当成公开带宽承诺；完整遍历所有资源的理论上界约为 139.2 MiB。
...
主分支只追踪前端、导出器、审计脚本和不足 1 KiB 的 `packaging/pages-data.json`。76.7 MiB ZIP 存放在独立的 Hugging Face 文件仓库；锁定清单记录不可变 revision、大小和 SHA-256，是构建使用版本的唯一权威来源。文件下载不依赖或启动 Space 容器。
