...
5. workflow 上传 ZIP、生成构建来源证明并创建 GitHub Release；GitHub Release 成功后，以同一 ZIP 生成固定路径、大小和 SHA-256 清单，并把最小 Docker Space 运行上下文同步到 Hugging Face；
6. 在仓库设置中为后续 Release 启用 immutability，锁定 tag 与资产。

HF 同步始终通过官方写入端点完成，用户下载才使用 `https://hf-mirror.com`。版本 ZIP、清单与 Space 运行文件在同一提交中更新，不会先公布一个尚未上传完成的安装包。同步只替换 Dockerfile、应用 wheel、编译前端和公开配置，保留独立上传的 `resources/default.ra2pack` 与历史 `releases/`。公共发行包只内嵌 Space 仓库标识和镜像地址，不包含 HF token、邮箱或本机配置。
