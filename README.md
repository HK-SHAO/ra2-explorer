...
统一搜索只保留可勾选的“单位 / 声音”目标，按 `Ctrl+K` 可从任意页面直接聚焦。关键词和补全在单位、语音、音效页面间保持一致，当前浏览分类只提高相关结果的排序权重，不会形成隐藏过滤条件。按回车或点击“搜索”会进入完整结果页，再按单位类型、阵营、声音类型与用途筛选结果；集合上方的浏览筛选仍只作用于正常浏览列表。

## 下载与磁盘占用

当前 Windows x64 本地包解压后约 56 MiB，ZIP 约 34 MiB。首次导入直接关联本机游戏目录，不会再复制数百 MiB 的 MIX，因此普通用户只需下载发行 ZIP。

Pages 精简资料解包后约 139.5 MiB，固定数据 ZIP 约 76.7 MiB，但浏览器不会下载 ZIP 或在启动时读取整站。单位页只先请求约 471 KiB 的当前语言单位目录和当前分类的一张缩略图图集，不再为侧栏计数提前读取完整声音目录，也不会为几十张卡片分别承担网络往返。高频数据包解包约 7.0 MiB，浏览器仍只通过 CDN 读取其中实际需要的单个文件。模型、详情和声音继续按操作加载；完整遍历全部内容时流量才可能接近整站体积。
