...
项目同时提供 Hugging Face Docker Space。它运行与本地版相同的 React 界面、Python 格式层和 FastAPI 查询接口，资料来自经过校验的 `.ra2pack`；不会把原始游戏归档或游戏程序放进容器。在线版是只读的，因此隐藏本机目录扫描、资源包导入/导出和参考数据写入操作。

纯静态 Space 只能承载前端外壳，无法直接运行当前的 Python 解析器、SQLite 语义索引和动态媒体接口，也不能绕过浏览器权限读取任意 Windows 游戏目录。因此当前采用 Docker Space，而不是把功能削减成静态演示。具体边界见 [Hugging Face 部署说明](docs/HUGGINGFACE.md)。
