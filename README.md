...
Pages 精简资料解包后约 140.0 MiB，固定数据 ZIP 约 76.8 MiB，但浏览器不会下载 ZIP 或在启动时读取整站。单位页先请求约 471 KiB 的当前语言单位目录、当前分类卡片图集和约 175 KiB 的当前角度搜索图集；搜索补全中的全部单位共用这张已预载的小图，不再产生逐项图片请求。侧栏计数无需提前读取完整声音目录，几十张卡片也无需分别承担网络往返。模型、详情和声音继续按操作加载；完整遍历全部内容时流量才可能接近整站体积。
