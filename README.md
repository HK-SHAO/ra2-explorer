...
Pages 精简资料解包后约 139.2 MiB，固定数据 ZIP 约 76.7 MiB，但浏览器不会下载 ZIP 或在启动时读取整站。单位页只先请求约 471 KiB 的当前语言单位目录和当前分类的一张缩略图图集，不再为侧栏计数提前读取完整声音目录，也不会为几十张卡片分别承担网络往返。高频数据包解包约 6.8 MiB，浏览器仍只通过 CDN 读取其中实际需要的单个文件。模型、详情和声音继续按操作加载；完整遍历全部内容时流量才可能接近整站体积。
