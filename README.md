...
集合区按当前类型提供名称、说明、体积、造价、生命值等有效排序；造价与生命值均可升序或降序排列，缺失数值稳定置于末尾。list 使用双列高密度浏览，grid 用于视觉识别，两者共用查询、筛选、选中与增量分页状态。单位 grid 卡片只保留居中的缩略图和名称，SHP 图像被约束在固定预览框内并保留底部安全区；同类型同名单位会在名称后以低对比度阵营或精确单位 ID 区分。声音分组的主标题与 ID/附加信息使用单行省略布局。规则中的所有单位都会进入列表，无法预览的对象作为解析状态反馈，不再被静默过滤。页面会记住资料源、分类、筛选、选择、导航折叠、布局尺寸及列表/详情滚动位置。
