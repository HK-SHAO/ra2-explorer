...
页面左侧使用统一树状导航：“单位”下直接选择载具、航空器、步兵和建筑；动画按 SHP、HVA 和视频分层，语音与音效按单位、EVA、战役、战斗和环境事件继续细分，不再设置“资源 / 单位”切换或重复的原始模型分类。声音分类来自 RULES/ART/SOUND/EVA/CSF 的实际引用关系，而不是 WAV、AUD 或 BAG 容器类型；列表同时显示可检索的台词、事件或任务说明，单击即可播放。地图、图像、地形、规则文本及其他底层格式可在显示设置中按需启用。

中栏按当前类型提供格式或语义标签以及名称、说明、体积、造价、生命值等有效排序；list/grid 共用查询、筛选、选中与增量分页状态，单位的阵营与国家筛选彼此独立于载具、航空器、步兵和建筑分类。详情根据类型切换为三维模型、动画顺序播放或全帧网格、图像、音频、视频、地图布局或文本，也可以弹出为独立浏览器窗口。
