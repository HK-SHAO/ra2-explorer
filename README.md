...
“设置”集中提供显示与载入偏好、原版目录解析、`.ra2pack` 导入/导出和用户可选的更新检查。卡片和模型缓存由应用自动调度：进入单位分类后持续预取该分类的缩略图，已访问分类在当前会话中直接复用；交互式模型只预测当前单位附近的少量项目，并使用有界缓存。用户不需要管理性能实现，也不会在启动时一次载入约数百 MiB 派生数据。已生成的索引、关联和浏览器预览可以导出为不含原始游戏文件的 `.ra2pack`，用于备份和迁移；格式边界见 [派生资源包说明](docs/RESOURCE_PACKS.md)。
