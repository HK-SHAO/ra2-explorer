...
集合区按当前类型提供名称、说明、体积、造价、生命值等有效排序；list 使用双列高密度浏览，grid 用于视觉识别，两者共用查询、筛选、选中与增量分页状态。单位 grid 卡片只保留缩略图和名称；声音分组的主标题与 ID/附加信息使用单行省略布局。规则中的所有单位都会进入列表，无法预览的对象作为解析状态反馈，不再被静默过滤。页面会记住资料源、分类、筛选、选择、导航折叠、布局尺寸及列表/详情滚动位置。详情根据类型切换为 VPL 光照三维模型、单位动作顺序播放或全帧网格、按非透明内容适配/缩放/平移的图像、紧凑音频、视频、地图布局或文本。默认采用上方集合、下方宽屏详情；显示设置可切换为集合与窄屏详情左右排列，两种布局都可拖动分隔条调整详情大小，独立窗口始终采用带滚动条的宽屏详情。宽屏预览只填充详情的可用高度，不会被右侧长内容撑大或持续改变 canvas 尺寸。单位详情使用“声音 / 动画 / 数据” tabs，声音资产使用“播放 / 关联 / 数据” tabs；单位声音按选中、移动、攻击等事件分组，动画点击后按对应事件的帧区间在主预览区播放，标签、规则属性和资源文件位于数据 tab 中并默认展开。
